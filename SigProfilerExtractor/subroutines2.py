...
def BootstrapCancerGenomes(genomes, seed=None):
    ...
    # Leer K activo del archivo
    k_file = os.path.join(os.path.dirname(__file__), "_current_k.txt")
    with open(k_file, 'r') as f:
        k = int(f.read().strip())

    # Leer SA y U de disco
    residuals_dir = os.path.join(os.path.dirname(__file__), "_residuals")
    SA = np.load(os.path.join(residuals_dir, f"SA_k{k}.npy"))
    U  = np.load(os.path.join(residuals_dir, f"U_k{k}.npy"))

    # Bootstrap simple de columnas de U
    rng = seed if seed is not None else np.random.default_rng()
    cols = rng.choice(U.shape[1], size=U.shape[1], replace=True)
    U_boot = U[:, cols]

    M_prime = SA + U_boot
    M_prime = np.clip(M_prime, 0, None)

    return pd.DataFrame(M_prime, index=genomes.index, columns=genomes.columns)
...
def pnmf(
    ...
    tic = time.time()
    # Escribir K activo en disco para bootstrap residual
    with open(os.path.join(os.path.dirname(__file__), "_current_k.txt"), 'w') as f:
        f.write(str(totalProcesses))
